...
// == Internal Crates
use fxv_diff_slint::HighlightKind;
use fxv_diff_slint_tests::{harness, harness_with_highlights, highlight, highlight_to_end, settle};
...
fn a_highlight_covers_the_columns_it_names() {
    let h = harness_with_highlights(10, &[highlight(0, 4..9, HighlightKind::Selection)]);
    settle();
...
fn several_highlights_on_one_row_are_all_drawn() {
    let h = harness_with_highlights(
        10,
        &[
            highlight(0, 0..3, HighlightKind::Selection),
            highlight(0, 10..12, HighlightKind::Marked),
        ],
    );
    settle();
...
fn a_highlight_covering_the_line_ending_reaches_the_pane_edge() {
    ...
    let h = harness_with_highlights(10, &[highlight_to_end(0, 4, HighlightKind::Selection)]);
    settle();
...
fn a_highlight_sits_after_the_gutter() {
    ...
    let h = harness_with_highlights(10, &[highlight(0, 0..2, HighlightKind::Selection)]);
    settle();
